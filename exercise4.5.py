...
# Example further tests

# 1. Compare number of tokens before and after cleaning
# print("Tokens in first file before cleaning:", len(data[0].split()))
# print("Tokens in first file after cleaning:", len(data_clean[0]))

# 2. Show first 10 tokens before and after cleaning
# print("First 10 tokens before cleaning:", data[0].split()[:10])
# print("First 10 tokens after cleaning:", data_clean[0][:10])

# 3. Show tokens removed by cleaning
original_tokens = set(data[0].split())
cleaned_tokens = set(data_clean[0])
removed_tokens = original_tokens - cleaned_tokens
# print("Tokens removed by cleaning:", removed_tokens)

# Most common words before cleaning (raw text, split by whitespace)
raw_words = data[0].split()
# print("Most common words before cleaning:", Counter(raw_words).most_common(10))

# Most common words after cleaning (cleaned tokens)
cleaned_words = data_clean[0]
...
# train the model
model = Word2Vec(sentences=data_clean, vector_size=200,
                 window=3, min_count=3, workers=4, epochs=5, sg=1)
...
# returns a list with the top fifteen words used in similar contexts to the word "milk"
print("\nVocabulary size (model2):", len(model.wv))
print("Most similar words to 'sugar' (model):",
      model.wv.most_similar('sugar', topn=5))

# returns the top ten most similar words to "recipe" that are dissimilar from "milk"
print("\nVector for 'milk':", model.wv.get_vector('milk'))

# returns the top ten most similar words to both "recipe" and "milk"
print("\nMost similar words to both egg and milk")
print(model.wv.most_similar(positive=["egg", "milk"], topn=10))

# returns a cosine similarity score for the two words you provide
print("\nWord that doesn't match ['milk', 'cream', 'butter', 'hammer']:",
      model.wv.doesnt_match(['milk', 'cream', 'butter', 'hammer']))

# returns a prediction for the other words in a set containing the words "flour," "eggs," and "cream"
print("\nWord Mover's Distance between 'milk cream' and 'butter sugar':",
      model.wv.wmdistance(['milk', 'cream'], ['butter', 'sugar']))
# displays the number of words in your model's vocabulary

model_cbow = Word2Vec(sentences=data_clean, window=5,
                      min_count=3, workers=4, epochs=5, sg=0)
print("\nMost similar words to 'bake' (CBOW):",
      model_cbow.wv.most_similar('bake', topn=5))


# Explore the Word2Vec class and KeyedVectors methods
# print("\nHelp on Word2Vec:")
# help(Word2Vec)

# print("\nHelp on KeyedVectors (model.wv):")
# help(model.wv)
# print("\nVocabulary size:")
# print(len(model.wv))
# print(help(Word2Vec))
